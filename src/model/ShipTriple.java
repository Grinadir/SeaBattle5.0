...
public class ShipTriple implements InterfaceShip {
    ...
    public boolean make(int x, int y) {
        if (isFirstRectangle()) {
            ...
            engine.setSaveX(x);
            engine.setSaveY(y);
            engine.setCountOfRectangleForTriple(engine.getCountOfRectangleForTriple() + 1);
            return true;
            ...
            if (engine.getSaveX1() == x
                ...
                xNO3 = x;
                yNO3 = y;
                engine.setCountOfRectangleForTriple(0);
    ...
    private boolean isThirtyRectangle(int x, int y) {
        return (engine.getSaveX() == x || engine.getSaveY() == y) && engine.getCountOfRectangleForTriple() == 2
                && engine.getThreeAmount() != -1;
    }

    private boolean isSecondRectangle(int x, int y) {
        return (xNO1 == x || yNO1 == y) && engine.getCountOfRectangleForTriple() == 1 && (xNO1 == x + 1 || yNO1 == y + 1 || xNO1 == x - 1 || yNO1 == y - 1)
                && engine.getThreeAmount() != -1;
    }

    private boolean isFirstRectangle() {
        return engine.getCountOfRectangleForTriple() == 0;
    }
}
